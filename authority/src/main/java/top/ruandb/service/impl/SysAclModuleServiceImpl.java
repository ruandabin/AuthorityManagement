...
public class SysAclModuleServiceImpl implements SysAclModuleServiceI {
	...
	public void addSysAclModule(SysAclModule sysAclModule) {
		...
		sysAclModule.setOperateIp(IpUtil.getRemoteIp(RequestHolder
	...
	public void updateSysAclModule(SysAclModule sysAclModule) {
		BeanValidator.check(sysAclModule);
		SysAclModule beforSysAclModule = sysAclModuleMapper
				.selectByPrimaryKey(sysAclModule.getId());
	...
	private void updateWithChild(SysAclModule before, SysAclModule after) {
		...
		if (!before.getLevel().equals(after.getLevel())) {
			List<SysAclModule> beforeChild = sysAclModuleMapper
					.selectAllChildSysAclModule(before);
			if (CollectionUtils.isNotEmpty(beforeChild)) {
				for (SysAclModule sysAclModule : beforeChild) {
					if (sysAclModule.getLevel().indexOf(before.getLevel()) == 0) {
						sysAclModule.setLevel(after.getLevel().concat(
								sysAclModule.getLevel().substring(
										before.getLevel().length())));
						sysAclModuleMapper
								.updateByPrimaryKeySelective(sysAclModule);
					}
				}
			}
		}
	...
	private String calculateLevel(SysAclModule sysAclModule) {
		...
		if (sysAclModule.getParentId() != null
				&& sysAclModule.getParentId() != 0) {
			String parentLevel = sysAclModuleMapper.selectByPrimaryKey(
					sysAclModule.getParentId()).getLevel();
			return StringUtils.join(parentLevel, SEPARATOR,
					sysAclModule.getParentId());
		} else {
			return ROOT;
		}
	}
	...
	private void transform2Tree(List<SysAclModuleDto> fList,
			Multimap<String, SysAclModuleDto> multimap) {
		for (int i = 0; i < fList.size(); i++) {
			SysAclModuleDto sysAclModuleDto = fList.get(i);
			String sunLevel = sysAclModuleDto.getLevel().concat(SEPARATOR)
					.concat(sysAclModuleDto.getId().toString());
